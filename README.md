...
- [로컬 실행](#로컬-실행)
- [HTTPS 실행](#https-실행)
- [Cloudflare Tunnel 외부 접속](#cloudflare-tunnel-외부-접속)
- [기능 확인 순서](#기능-확인-순서)
- [검증 명령](#검증-명령)
- [종료와 초기화](#종료와-초기화)
- [보안과 Git 제외 항목](#보안과-git-제외-항목)
- [제한 사항](#제한-사항)
...
- WebSocket gateway: `backend/app/gateway/router.py`, `backend/app/gateway/events.py`
- Voice signaling/backend state: `backend/app/gateway/voice_service.py`
- Frontend app shell: `frontend/src/App.vue`
- Friends/DM UI: `frontend/src/components/FriendsHome.vue`, `frontend/src/components/DirectMessageView.vue`
- Server/channel UI: `frontend/src/components/ServerRail.vue`, `frontend/src/components/ChannelSidebar.vue`, `frontend/src/components/ChatView.vue`
- Voice UI: `frontend/src/components/VoicePanel.vue`
- WebRTC client logic: `frontend/src/composables/useVoiceRtc.ts`, `frontend/src/composables/voicePeerConnections.ts`, `frontend/src/composables/voiceMedia.ts`
- Settings UI: `frontend/src/components/SettingsView.vue`
...
1. 회원가입 또는 개발 사용자 로그인
2. 다른 브라우저 세션에서 두 번째 계정 로그인
3. 친구 추가와 친구 요청 수락
4. DM 생성과 DM 메시지 송수신
5. DM 개인 통화 시작/종료
6. 서버 생성
7. 초대 코드 생성과 다른 계정의 서버 참여
8. 서버 텍스트 채널 메시지 송수신
9. 서버 음성 채널 참여
10. 마이크 음소거, 소리 차단, 입력/출력 설정 확인
11. 화면 공유 시작/중지
12. 서버 멤버 권한 변경
13. 서버 나가기 또는 권한이 있는 계정의 서버 삭제
