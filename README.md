...
- 음성 입력은 브라우저 오디오 처리 옵션, 입력 감도 게이트, RNNoise WASM 기반 잡음 제거를 조합해 처리합니다. 기본값은 긴 발성이 끊기지 않도록 안정성을 우선하고, 필요할 때 설정 화면에서 RNNoise 잡음 제거와 입력 감도 게이트를 켤 수 있습니다.
...
처음 접속하면 회원가입 화면에서 계정을 만들거나 `테스트 계정으로 시작` 버튼으로 시연용 관리자 계정에 바로 접속할 수 있습니다. 두 개 이상의 브라우저 세션 또는 서로 다른 기기에서 다른 계정으로 접속하면 친구, DM, 서버 메시지, 음성 채널 흐름을 확인할 수 있습니다.
...
`테스트 계정으로 시작` 버튼은 로컬 개발과 시연용 관리자 계정으로 바로 접속하는 기능입니다. 이 계정은 `admin` 이름을 사용하며 기본 서버의 소유자 권한을 가집니다. 테스트 계정은 로그아웃할 때 테스트 계정의 DM, 친구, 서버 작업 내역이 초기 상태로 정리됩니다. 일반 회원가입 흐름을 확인할 때는 `회원가입` 탭을 사용하세요.

일반 회원가입 계정은 로그아웃해도 자동 초기화되지 않습니다. 전체 PostgreSQL 데이터를 깨끗하게 지우고 처음 상태로 되돌리려면 아래 초기화 명령을 사용합니다.
...
터미널에 출력되는 `https://*.trycloudflare.com` 주소로 외부 기기에서 접속합니다. 이 방식은 `https://localhost:5173`에 직접 연결되므로 로컬 수정사항이 Cloudflare URL에도 즉시 반영됩니다. HTTPS/Cloudflare 확인 중에는 일반 `npm run docker:up`으로 다시 띄우지 말고 `npm run docker:up:https:detached` 상태를 유지하세요. 일반 HTTP compose로 다시 띄우면 Cloudflare dev 터널의 HTTPS origin과 맞지 않아 접속이 끊길 수 있습니다.

정적 빌드 결과를 확인해야 하는 경우에는 아래 방식을 사용할 수 있습니다. 이 방식은 실시간 개발 반영용이 아니라 빌드 산출물 확인용입니다.
...
`npm run docker:down`은 PostgreSQL volume을 보존합니다. 테스트 데이터를 모두 지우고 처음 상태로 되돌릴 때만 `docker compose down -v`를 사용합니다. `테스트 계정으로 시작` 계정은 앱에서 로그아웃할 때 해당 테스트 계정의 작업 내역만 초기화됩니다.

## 주요 구현 기능

- 회원가입, 로그인, 테스트 계정 시작과 테스트 계정 로그아웃 초기화
