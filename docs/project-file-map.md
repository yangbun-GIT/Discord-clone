...
- `frontend/src/components/DirectMessageView.vue`
  ...
  - Keeps all DM messages left-aligned while distinguishing current-user messages
    ...
    DM message `created_at` values for message time display, the intro-owned first
    date divider, and additional per-day dividers between messages. The intro plus
    message rows are wrapped by one `dm-thread-stack` so intro actions and first
