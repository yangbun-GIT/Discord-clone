...
## 2026-06-20 DM 메시지 후속 수정

- 내 메시지와 상대 메시지를 모두 좌측 정렬로 통일했습니다.
- 내 메시지는 별도 배경/좌측 포인트 라인으로만 구분하고, 삭제 버튼은 내 메시지에서만 노출됩니다.
- DM 메시지를 전송한 뒤 입력 칸 focus가 유지되어 연속 입력이 가능합니다.
- DM 목록의 각 대화에는 `대화 닫기` 버튼이 추가되었습니다.
  - 이 기능은 대화를 모든 사용자에게서 삭제하지 않습니다.
  - 현재 사용자 목록에서만 숨기며, 같은 상대에게 다시 DM을 시작하면 기존 대화가 다시 표시됩니다.
